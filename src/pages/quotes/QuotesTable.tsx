...
export const QuotesTable = ({ table, isTableLoading } : PropTypes) => {
    ...
    return (
        <table className={`mt-15 pb-44 transition-all duration-200 min-h-[500px] w-full`}>
                ...
                {table.getHeaderGroups().map(headerGroup => (
                    <tr key={headerGroup.id} className={'h-36 bg-lightest-gray'}>
                        {headerGroup.headers.map(header => (
                            ...
                            >
                                <div
                                    className={`flex items-center justify-center ${header.column.getCanSort() ? 'cursor-pointer select-none' : '' }`}
